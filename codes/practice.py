...
shun = visual.ImageStim(
    ...
    color=[1,1,1], colorSpace='rgb', opacity=0.25,
...
def practice_RS1(time=30.0, id=1, flag=0):
    '''
    practice RS1 stimuli
    '''
...
def practice_RS2(time=30.0, id=1, flag=0):
    '''
    practice RS2 stimuli
    '''
    sound_1.setSound('B', secs=0.5, hamming=True)
    sound_1.setVolume(1, log=False)
    sound_2.setSound('C', secs=0.5, hamming=True)
...
#显示QD
def practice_QD(time=30.0, freq=30, id=1, flag=0):
    '''
    show QD stimuli
    '''
    sound_1.setSound('B', secs=0.5, hamming=True)
    ...
    # keep track of which components have finished
    components = [square1, square2, square3, square4, pic, cue]
    init_routine(components)
    # reset timers
    t = 0
    _timeToFirstFrame = win.getFutureFlipTime(clock="now")
...
def run_trial(type,re, id):
    ...
    if type == 'RS1':
        ...
        practice_RS1(time=10.0, id=id, flag=flag)            #原本为30s
    elif type == 'RS2':
        show_pic(focus,time=1.0,id=1)
        practice_RS2(time=30.0,id=id, flag=flag)
    elif type == 'QD':
        show_pic(focus,time=1.0,id=1)
        practice_QD(time=3.0,id=id, flag=flag)
    
    show_text1('rest', time = 5.0) # rest for 25s
    

# 9 trials for each practice
def domi(type):
    re=np.random.randint(0, 100, size=(9,));
    show_text('natural rate', time = 2.0)
    run_trial(type,re,id=1)
    show_text('fast rate', time = 2.0)
    run_trial(type,re, id=2)
    show_text('slow rate', time = 2.0)
    run_trial(type,re, id=3)
    show_text('fast rate', time = 2.0)
    run_trial(type,re, id=4)
    show_text('natural rate', time = 2.0)
    run_trial(type,re, id=5)
    show_text('fast rate', time = 2.0)
    run_trial(type,re, id=6)
    show_text('slow rate', time = 2.0)
    run_trial(type,re, id=7)
    show_text('natural rate', time = 2.0)
    run_trial(type,re, id=8)
    show_text('slow rate', time = 2.0)
    run_trial(type,re, id=9)



# ====== experiment starts here ======

# practice three stimuli
 
show_text('Multistable Perception', time=3.0)
show_text('practice stimu 1', time=3.0)
domi('RS1')
show_text('practice stimu 2', time=3.0)
domi('RS2')
show_text('practice stimu 3', time=3.0)
domi('QD')
# after experiment
f.close()
win.flip()
