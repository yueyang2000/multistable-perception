...
# This code will run before the experiment window is opened, after importing all of the necessary packages 
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/yueyang/yiqunyang/SRT_认知/learn_psychopy/mpstr.py',
...
for thisTrial in trials:
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if movie.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > movie.tStartRefresh + 3-frameTolerance:
                # keep track of stop time/frame for later
    ...
    for thisComponent in sphere2Components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    trials.addData('movie_2.started', movie_2.tStartRefresh)
    trials.addData('movie_2.stopped', movie_2.tStopRefresh)
    ...
    for thisComponent in squareComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    trials.addData('square1.started', square1.tStartRefresh)
    trials.addData('square1.stopped', square1.tStopRefresh)
    trials.addData('square2.started', square2.tStartRefresh)
    trials.addData('square2.stopped', square2.tStopRefresh)
    trials.addData('square3.started', square3.tStartRefresh)
    trials.addData('square3.stopped', square3.tStopRefresh)
    trials.addData('square4.started', square4.tStartRefresh)
    trials.addData('square4.stopped', square4.tStopRefresh)
